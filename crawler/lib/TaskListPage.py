...
class TaskListPage:
    class Task:
        ...
        def __init__(self, table_row: Tag) -> None:
            ...
            time_limit_sec_match: Optional[Match[str]] = (
                self.time_limit_sec_pattern.search(time_limit_sec_str)
            )
            ...
            memory_limit_mb_match: Optional[Match[str]] = (
                self.memory_limit_mb_pattern.search(memory_limit_mb_str)
            )
            if memory_limit_mb_match is not None:
                self.memory_limit_mb = int(memory_limit_mb_match.group(1))
            else:
                assert memory_limit_mb_str.strip() != "0 KiB", f"{memory_limit_mb_str=}"
